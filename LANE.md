...
- **Tests.** Run each package serially on an idle machine, never while `xcodebuild` is running (a parallel run produced 22+ spurious timeouts):
  `swift test` (rules 53) · `swift test --package-path Tests/BoardTests` (271, XCTest — its swift-testing runner reports "0 tests", read the XCTest "Executed N tests" line instead) · `Tests/MatchTests` (128) · `Tests/StyleTests` (44) · `Tests/ShellTests` (270, `--no-parallel`) · `Tests/SettingsTests` (37) · `Tests/BotTests` (68, ~7 min) · `Tests/OnlineTests` (147, 1 known pre-existing issue at `WholeMatchScript.swift:98`, live cases skip without a key) · `Tests/AudioTests` (19) · `Tests/AccountTests` (21) · `Tests/FriendsTests` (54).
...
- **Always run ShellTests as `swift test --package-path Tests/ShellTests --no-parallel`.** Its cases are `@MainActor` and poll at 1ms, so running them concurrently starves them against their own 10s deadlines — the same tree gave 12 issues in parallel and 4 serially. Item 5 fixed those 4, so the suite is now **266 with 0 issues**: any red is yours. Never read the collected count as the result — round 2 did, and recorded a failing suite as green.
