...
- **Tests.** Run each package serially on an idle machine, never while `xcodebuild` is running (a parallel run produced 22+ spurious timeouts):
  `swift test` (rules 53) · `swift test --package-path Tests/BoardTests` (271, XCTest — its swift-testing runner reports "0 tests", read the XCTest "Executed N tests" line instead) · `Tests/MatchTests` (128) · `Tests/StyleTests` (42) · `Tests/ShellTests` (264) · `Tests/SettingsTests` (36) · `Tests/BotTests` (68, ~7 min) · `Tests/OnlineTests` (147, 1 known pre-existing issue at `WholeMatchScript.swift:98`, live cases skip without a key) · `Tests/AudioTests` (19) · `Tests/AccountTests` (16) · `Tests/FriendsTests` (49).
...
- **ShellTests goes red under machine load, and it is not your item.** Several of its cases wait on "the opening deal/board" against a fixed ~11s WALL-CLOCK deadline, so at a 5-minute load average above roughly 30 it returns 14–22 timeout issues on code it never compiled. Before believing a ShellTests red: run `xcrun simctl shutdown all`, check the load average, and re-run it alone. Confirm the file you changed is even in its target — `Tests/ShellTests/Package.swift` points the Style target at `StyleSrc` and excludes the view files, so most Style and Friends edits are invisible to it.
