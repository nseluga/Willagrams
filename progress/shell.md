...
| Anyone who learns your friend code can read your live invitations | fixed — Invitations now travel on a private line the server polices: it will only let you listen to invitations addressed to you, and only let someone you have accepted as a friend send you one. A stranger who works out your id can no longer sit and watch your invitations go past, nor take the game slot meant for your friend. Needs `scripts/apply-0005-live.sh` run against the live project **before** a build carrying this change ships — until then the server denies the new private line and no invitation arrives at all. |
