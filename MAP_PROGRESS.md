...
| online | nate | lane/online | **round 1 done** — merged 2026-09-02 via /merge-lane, PR #3, items 1–7 at `be67ecf`; archive `progress/online.md` + `progress/online-lane-plan.md`; OnlineTests 26 → 124. Ships the concrete `SupabaseBackend` (auth + profile, friends, matches, outcome), `RealtimeMatchTransport`, `MatchOutcomeRecorder` and the `OnlineMatch` façade. Item 8 (real Sign in with Apple) sits below the stop marker, untouched, still waiting on the Apple Developer membership. **Two things stay open.** The whole seam is `unwired:` — no screen constructs any of it; that is `account` and `friends`. And **nothing live has ever been executed**: all 22 gated cases are written against the real API and proven to fire (a junk key returns a real HTTP 401 from the project), but `SupabaseConfig.anonKey` reads `SUPABASE_ANON_KEY` from the environment with an empty-string fallback, so no key was reachable and none would exist in a Release build either. Anonymous sign-ins also remain off on the hosted project. Both are owed before online is called from any screen — see `progress/online.md` |
