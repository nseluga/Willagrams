...
| audio | nate | lane/audio | **in progress** — branch cut from `f752d0d` 2026-08-25. Depends only on style, which merged. Scope settled in MAP's "Decided — what the `audio` lane is and is not": the seam has zero call sites and wiring them is a **shell round 3** item, not this lane's; mute is sound-only and board haptics deliberately stay outside it; the mute *control* is settings' and only the mute *state* is here. Build constraint: `Tests/AudioTests/AudioSrc` symlinks the whole audio directory and compiles it for macOS, so any UIKit or AVFoundation file must be split out or excluded |
