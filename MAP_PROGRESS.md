...
| online | nate | — | not started, and **now unblocked at the database**. `0001_init.sql` and `0002_participant_lookup.sql` are applied to project `ynkayuwwrifluhhqnrjc`; both SQL fixtures pass against it, twice, in either order. **No `/foundation` amendment is needed:** the app is iOS-only and ships through the App Store, so every player already has an Apple ID, and `signInWithApple` being the protocol's only route to a session is the right shape rather than a gap. Only enabling the capability is deferred, on the paid Apple Developer membership; `FakeBackend` already implements `signInWithApple`, so the lane is buildable before that |
